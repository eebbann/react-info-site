...
export default function I({
  ...
  const [tableColumnVisibilityColumnExtensions] = useState([
    { columnName: "ac mapping code", togglingEnabled: false },
    { columnName: "dc mapping code", togglingEnabled: false },
  ]);
  useEffect(() => {
    setRows(data);
  }, [data]);
  ...
  const [filteringColumnExtensions] = useState([
	...
	const onSave = (workbook) => {
		workbook.xlsx.writeBuffer().then((buffer) => {
			saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'inHovate_data.xlsx');
		});
	};
  ...
  const commitChanges = async ({ added, changed, deleted }) => {
    try {
      ...
      if (deleted) {
        const id = deleted[0];
        await axios
          .delete(`/api/property/data/coa/${propertyId}/${id}`, {
            headers: {
              Authorization: `Bearer ${userToken}`
            }
          })
